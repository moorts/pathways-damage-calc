...
export function calculatePathways(
  ...
  const baseDamage = calculateBaseDamagePathways(
    gen,
    attacker,
    defender,
    basePower,
  ...
  if ((move.dropsStats && move.timesUsed! > 1) || move.hits > 1) {
    ...
    for (let times = 1; times < numAttacks; times++) {
      ...
      const newBaseDamage = calculateBaseDamagePathways(
        gen,
        attacker,
        defender,
        newBasePower,
      ...
      const newFinalMods = calculateFinalModsPathways(
        gen,
        attacker,
        defender,
        move,
...
export function calculateBasePowerPathways(
  ...
  const bpMods = calculateBPModsPathways(
    gen,
    attacker,
    defender,
    move,
...
export function calculateAttackPathways(
  ...
  const atMods = calculateAtModsPathways(gen, attacker, defender, move, field, desc);
  attack = OF16(Math.max(1, pokeRound((attack * chainMods(atMods, 410, 131072)) / 4096)));
  return attack;
}

export function calculateAtModsPathways(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
...
export function calculateDefensePathways(
  ...
  const dfMods = calculateDfModsPathways(
    gen,
    attacker,
    defender,
    move,
...
export function calculateDfModsPathways(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
...
function calculateBaseDamagePathways(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  basePower: number,
...
export function calculateFinalModsPathways(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
